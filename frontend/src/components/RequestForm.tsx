...
export function RequestForm({ channelSlug }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(`Request submitted! Position: #${result.position}`);
      setCaption("");
      setLyrics("");
      setBpm("");
    } catch (e) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-gray-800 rounded-lg p-4 space-y-3">
      <div className="text-sm font-medium text-gray-300">Request a Track</div>
      ...
      <textarea
        ...
        placeholder="Lyrics (optional, use [Verse], [Chorus] tags)"
        rows={3}
